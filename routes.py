...
def forecast_summary():
    ...
    if request.method == 'POST':
        try:
            ...
            if not labels:
                flash(f"⚠️ Not enough sales data for '{product.name}'. Please record at least 2 past transactions before generating a forecast.", "warning")
            else:
                chart_ready = True
...
def add_transactions():
    ...
    #🕒 Timestamp Validation
        ...
        if duplicate == "yes":
            ...
            db.session.add(duplicate_tx)
            db.session.commit()
            flash("Transaction saved AND duplicated in history!", "success")
        else:
